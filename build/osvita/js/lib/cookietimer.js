...
/// @update Vsevolod Kurochka 21.12.2017
...
CookieTimer.prototype.saveCookie = function()
{
		...
		var cookiedate = 
				$.cookies.set(cookiename, [this.name, this.duration, this.startTime].join(','),
											{ expiresAt: new Date(this.startTime.getTime() + (this.duration+300000)*10000000) });
}
...
CookieTimer.prototype.addToDocument = function()
{
		...
		$(".timerContainer").append("<div id='" + this.id + "' class='cookieTimer'>" + 
																"<div class='display-group'>" + 
																"<span class='v-timer__time time'>00:00</span>" +
																"</div>" + 
																"</div>");
		var obj = this;
...
CookieTimer.prototype.draw = function()
{
		...
		{
				$('.v-timer').addClass('v-timer_end');
				$('.v-timer .v-timer__title').html("Вы не успели забрать бонус!");
				$("#" + this.id).html("00:00");
				$('.v-bonus__shadow').slideUp('slow');
		}
}
